...
export const Sidebar: React.FC<SidebarProps> = ({ collapsed, onToggle }) => {
  ...
  const menuItems = [
    ...
    { id: 'compare', icon: Files, label: '多版本文件比对', path: 'https://newbidui.graybruce.cn/', isExternal: true },
  ...
  const isPathActive = (itemPath: string, isExternal?: boolean) => {
    ...
    <div className={`bg-white h-full border-r border-gray-200 transition-all duration-300 flex flex-col ${collapsed ? 'w-20' : 'w-64'}`}>
      ...
      <nav className="flex-1 py-4 px-3 space-y-1">
        {menuItems.map((item) => {
          ...
          const isActive = isPathActive(item.path, item.isExternal);
          return (
            <button
              key={item.id}
              onClick={() => {
